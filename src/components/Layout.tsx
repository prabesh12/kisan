...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <footer className="md:hidden fixed bottom-1 flex justify-center w-full z-50">
           <div className="bg-white px-2 pt-2 pb-2 z-50 shadow-2xl border border-gray-100 rounded-3xl mx-4 w-full max-w-md">
              ...
              {!isAuthenticated && (
                <Link
                  ...
                  className={`flex flex-col items-center p-2 text-gray-400 hover:text-primary-400 transition-all ${
                    location.pathname === '/login' ? 'text-primary-600 bg-primary-50 px-4 rounded-2xl' : ''
                  }`}
                >
                  <LogIn size={22} />
                  <span className="text-[9px] mt-1 font-bold uppercase tracking-tighter">{t('nav.login')}</span>
                </Link>
              )}
            </div>
          </div>
